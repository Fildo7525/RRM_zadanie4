...
{
	Eigen::MatrixXd pointOne(6, 6);
	Eigen::MatrixXd pointFinal(6, 1);
	Eigen::MatrixXd pointOneAParams(6, 1);
	pointOne << 1,   t0, std::pow(t0, 2),	 std::pow(t0, 3),	 std::pow(t0, 4),	 std::pow(t0, 5),
				0, 2*t0, std::pow(t0, 2),  3*std::pow(t0, 2),  4*std::pow(t0, 3),  5*std::pow(t0, 4),
				0,	  0,			   1,				6*t0, 12*std::pow(t0, 2), 20*std::pow(t0, 3),
				1,   t2, std::pow(t2, 2),	 std::pow(t2, 3),	 std::pow(t2, 4),	 std::pow(t2, 5),
				0, 2*t2, std::pow(t2, 2),  3*std::pow(t2, 2),  4*std::pow(t2, 3),  5*std::pow(t2, 4),
				0,    0,			   1,				6*t2, 12*std::pow(t2, 2), 20*std::pow(t2, 3);
	pointFinal << 0, 0, 0, 90, 0, 0;
